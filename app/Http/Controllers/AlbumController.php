...
class AlbumController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $album->author_type = $request->authortype;
       ...
       if ($request->authortype != '1') {
            ...
            if ($request->author) {
                ...
                foreach ($autorids as $key => $value) {
                    ...
                    $article_author = new AlbumAuthor();
                    $article_author->album_id = $id;
    ...
    {   
       ...
       $tags=  json_encode(DB::table('tags')
        ...
        $arrAuth = DB::table('album_author')
                            ...
                            ->select('album_author.author_id as id', 'authors.name')
       ...
       // $channels = AlbumController::getUserChannels($uid);
        return view('album.edit', compact('album','photos','tags','channels','postAs','authors'));
    ...
    {//Update album
        ...
        $oldAuthorType=$album->author_type;
       ...
       $images = explode(',', $request->uploadedImages);
            ...
            if ($request->authortype!='1') {
                ...
                if($oldAuthorType==$request->authortype)
                    $deleteOldAuthor=DB::table('album_author')->where('album_id', '=', $id)->whereNotIn('author_id',$autorids)->delete();
                else
                    $deleteOldAuthor=DB::table('album_author')->where('album_id', '=', $id)->delete();
                
                foreach ($autorids as $key => $value) {
                    if (DB::table('album_author')->where('album_id', '=', $id)->where('author_id', $value)->count() == 0) {
                        $article_author = new AlbumAuthor();
                        $article_author->album_id = $id;
                        $article_author->author_id = $value;
                        $article_author->save();

...
class ActivityLog extends Model {
    ...
    public function storeActivityCustom($section, $data, $extra_data=0) {

        switch ($section):
            case 'event-attendee':
                
                $speakerType=SpeakerType::find($data['attendeeType']);
                $message = 'Imported: <a href="/download/logs/'.  base64_encode($data['filename']).'">' . $data['filename'].'</a> as '.$speakerType->name.' for "'.$extra_data['title'].'" event.';
                if($data['noOfEscapped']){
                    $message.=' </br>Escapped rows file is: <a href="/download/logs/'.  base64_encode($data['escapped_filename']).'">'.$data['escapped_filename'].'</a>';
                }
                $data_to_store = $data;

                break;
            case 'attendee':
                 $message = 'Imported:<a href="/download/logs/'.  base64_encode($data['filename']).'">' . $data['filename'].'</a>';
                if($data['noOfEscapped']>0){
                    $message.=' </br>Escapped rows file is: <a href="/download/logs/'.  base64_encode($data['escapped_filename']).'">'.$data['escapped_filename'].'</a>';
                }
                $data_to_store = $data;
                break;

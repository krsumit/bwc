...
class Cron {
    ...
    function __construct() {
        ...
        $this->url='http://bwpeople.businessworld.in/';
    ...
    function sendMailAuthor() {
        ...
        $articlesResults = $this->conn->query("SELECT articles.article_id,articles.title,articles.publish_date FROM articles where  status ='P' and send_mail_status='0' and channel_id ='8'");
   ...
   function sendMail($id){
       $authorResults = $this->conn->query("SELECT articles.article_id,articles.title,articles.publish_date,article_author.*,authors.* FROM articles  JOIN article_author ON  article_author.article_id = articles.article_id JOIN  authors ON  authors.author_id = article_author.author_id where articles.article_id = $id ");
        ...
        $return_html .= '<title>BW People</title>';
        ...
        $return_html .= '<tr><td height="50"><a href="#"><img editable="true" width="250" src="http://d1s8mqgwixvb29.cloudfront.net/static_bwhotelier/images/BWH-Logo-300x72.jpg" alt="" border="0"></a></td></tr>';
        $return_html .= '</table>';
        ...
        $return_html .= '<tr><td width="590" style="font-size: 14px; color: #696a78; text-align: left; font-weight: normal; font-family: Helvetica, Arial, sans-serif; line-height: 26px;"><singleline>Your article titled <a href="'.$url.'" target="_blank">'.$articletitle.'&nbsp;(click here!)</a> has been published on BW People website.</singleline></td></tr>';
        ...
        $return_html .= '<tr><td style="font-family:Segoe,Segoe UI,DejaVu Sans,Trebuchet MS,Verdana,sans-serif !important;"><strong >BW People Editorial</strong></td> </tr>';
        $return_html .= '<tr><td width="590" height="30"></td></tr>';

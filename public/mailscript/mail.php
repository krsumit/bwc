...
class Cron {
    ...
    function sendMailAuthor() {
      ...
      // $articlesResults = $this->conn->query("SELECT articles.article_id,articles.title,articles.publish_date FROM articles where  status ='P' and article_id ='112799'");
   ...
   function sendMail($id){
       ...
       $authorResults = $this->conn->query("SELECT articles.article_id,articles.title,articles.publish_date,article_author.*,authors.* FROM articles  JOIN article_author ON  article_author.article_id = articles.article_id JOIN  authors ON  authors.author_id = article_author.author_id where articles.article_id = $id ");
	      ...
	      $publish_date=date('d-m-Y',strtotime($authorRow['publish_date']));
            ...
            //echo "https://api-ssl.bitly.com//v3/shorten?access_token=$bitly_acesss_token&longUrl=$urlenc"; exit;
            $data=file_get_contents("https://api-ssl.bitly.com//v3/shorten?access_token=$bitly_acesss_token&longUrl=$urlenc");
            $arr=json_decode($data);
            $updateDate = date('Y-m-d H:i:s');
            
            $updatearticlestmt = $this->conn->prepare("update articles set bitly_url=?,updated_at=? where article_id=?");
            $updatearticlestmt->bind_param('ssi',  $arr->data->url,$updateDate, $article_id);
...
function sendSms($url,$mob,$authorName,$articleTitle,$bitlyUrl){
    //$mob='9899415606';
    //$bitly_acesss_token='54faae0489a41d4c932f27cd7d5a060563bace93';
    ...
    ///$arr=json_decode($data);
    $message=urlencode("Hi $authorName, Your article : $articleTitle has been published. Url :".$bitlyUrl); 

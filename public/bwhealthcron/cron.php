...
class Cron {
    ...
    function migrateBwAuthor() {
        ...
        echo $this->message = '<h5 style="color:#009933;">' . $_SESSION['noofins'] . 'bwhealthauthors(s) inserted and ' . $_SESSION['noofupd'] . 'bwhealthauthors(s) updated.</h5>';
    }
function migrateFeature() {
	...
	///echo 'test';
        ...
        $updatecronstmt->execute();
        $updatecronstmt->close();
        echo $this->message = '<h5 style="color:#009933;">' . $_SESSION['noofins'] . 'bwhealthsponsorviewcount(s) inserted and ' . $_SESSION['noofupd'] . 'bwhealthsponsorviewcount   (s) updated.</h5>';
    } 
	...
	///echo 'test';
        ...
        $updatecronstmt->execute();
        $updatecronstmt->close();
        echo $this->message = '<h5 style="color:#009933;">' . $_SESSION['noofins'] . 'bwhealtharticleviewcount(s)inserted and ' . $_SESSION['noofupd'] . 'bwhealtharticleviewcount(s) updated.</h5>';
    } 
	...
	///echo 'test';
        ...
        $updatecronstmt->execute();
        $updatecronstmt->close();
        echo $this->message = '<h5 style="color:#009933;">' . $_SESSION['noofins'] . 'bwhealthevent(s) inserted and ' . $_SESSION['noofupd'] . 'bwhealthevent(s) updated.</h5>';
    }
...
function migrateBwTopics(){
        ...
        $cronresult = $this->conn->query("select start_time from cron_log where section_name='bwhealthtopics' order by  start_time desc limit 0,1") or die($this->conn->error);
        $condition = '';
        if ($cronresult->num_rows > 0) {  
            $cronLastExecutionTime = $cronresult->fetch_assoc()['start_time'];
            $condition = " and  (created_at>='$cronLastExecutionTime' or updated_at>='$cronLastExecutionTime')";
